...
const App: React.FC = () => {
  ...
  // Initialize App & Seed Data if Empty
  useEffect(() => {
    const loadData = async () => {
      let storedUser = db.getUser();
      let storedSubjects = db.getSubjects();
      
      if (!storedUser || storedSubjects.length === 0) {
        await db.seedDatabase();
        storedUser = db.getUser();
        storedSubjects = db.getSubjects();
      }
      
      setUser(storedUser);
      if (storedUser) applyTheme(storedUser.themePref);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 transition-colors duration-500 font-sans text-text">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start justify-between w-full md:w-auto">
            <div>
              ...
              <p className="text-text-muted mt-1 font-medium text-sm md:text-base tracking-wide uppercase">{user?.university}</p>
            </div>
            <button 
          ...
          <div className="flex gap-3">
            <button 
              ...
              className="flex items-center justify-center gap-2 bg-surface text-text-muted hover:text-accent border border-white/10 hover:border-accent/30 px-4 py-2.5 rounded-full transition-all"
              title="Wszystkie Dokumenty"
            >
              <FileText size={20} /> <span className="hidden md:inline">Dokumenty</span>
            </button>
             <button 
              onClick={() => setShowSettings(true)}
              className="hidden md:flex items-center justify-center gap-2 bg-surface text-text-muted hover:text-accent border border-white/10 hover:border-accent/30 px-4 py-2.5 rounded-full transition-all"
            >
              <Settings size={20} />
            </button>
            <button 
              onClick={() => setShowCreateSubject(true)}
              className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-accent text-white hover:bg-accent/90 px-6 py-2.5 rounded-full shadow-lg shadow-accent/20 transition-all font-medium"
            >
              <Plus size={20} /> Nowy Przedmiot
            </button>
          </div>
        </div>

        {/* Daily Quote Card */}
        <GlassCard className="text-center relative overflow-hidden min-h-[140px] md:min-h-[160px] flex items-center justify-center py-8">
          <div className="absolute top-4 left-4 opacity-10 text-primary">
             <Sparkles size={32} className="md:w-12 md:h-12" />
          </div>
          <div className="absolute bottom-4 right-4 opacity-10 text-primary rotate-180">
             <Sparkles size={32} className="md:w-12 md:h-12" />
          </div>
          <p className="font-hand text-2xl md:text-4xl text-transparent bg-clip-text bg-gradient-to-r from-primary via-accent to-secondary relative z-10 leading-tight p-2 md:p-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3 md:gap-4">
              <GlassCard className="text-center hover:scale-[1.02] transition-transform cursor-default p-4">
                <BookOpen className="w-5 h-5 md:w-6 md:h-6 mx-auto text-accent mb-2" />
                <p className="text-xl md:text-2xl font-bold font-serif">{subjects.length}</p>
              ...
              <GlassCard className="text-center hover:scale-[1.02] transition-transform cursor-default p-4">
                ...
                <p className="text-xl md:text-2xl font-bold font-serif">{totalDocs}</p>
              ...
              <GlassCard className="text-center hover:scale-[1.02] transition-transform cursor-default p-4">
                ...
                <p className="text-xl md:text-2xl font-bold font-serif">{totalEvents}</p>
                  ...
                  >
                    ...
                    <div className="p-5">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="font-bold text-lg font-serif group-hover:text-accent transition-colors">{sub.title}</h3>
                          <p className="text-sm text-text-muted font-medium">{sub.code}</p>
                        </div>
                        <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-accent group-hover:text-white transition-all">
                    ...
                    <button
                      ...
                      className="absolute bottom-5 right-5 p-2 rounded-full bg-red-500/10 text-red-500 transition-colors hover:bg-red-500 hover:text-white z-10"
          ...
          <div className="space-y-6">
            ...
            {/* Status Widget */}
            <div className="glass-panel rounded-2xl overflow-hidden p-5 bg-gradient-to-br from-accent/10 to-transparent border-accent/20">
            ...
            <GlassCard>
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {['Pn','Wt','Śr','Cz','Pt','Sb','Nd'].map((d, i) => (
                  <div key={i} className={`p-2 rounded-lg text-xs ${i === 3 ? 'bg-accent text-white font-bold' : 'text-text-muted hover:bg-white/5'}`}>
      ...
      {showGlobalDocs && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
          <GlassCard className="w-full max-w-4xl max-h-[85vh] !p-0 overflow-hidden shadow-2xl border border-white/20 flex flex-col">
            <div className="p-5 border-b border-white/10 flex justify-between items-center bg-white/5 shrink-0">
              <h2 className="text-xl font-serif font-bold text-primary flex items-center gap-2">
                <FileText size={24} className="text-accent" /> Baza Dokumentów
              </h2>
              <button 
                                ...
                                return (
                                    <div key={doc.id} className="p-3 bg-white/5 rounded-xl border border-white/10 flex items-start justify-between">
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-[10px] text-text-muted bg-white/5 px-1.5 py-0.5 rounded inline-block">
                                                {subjectName}
                                            </p>
                                        </div>
                                        {doc.isAnalyzed && <CheckCircle size={14} className="text-green-500 shrink-0" />}
                                    </div>
                                );
                            })}
                        </div>
